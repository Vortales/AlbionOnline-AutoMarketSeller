...
№ Важные параметры, для гибкой настройки auto_config.json

🔧 logic — Логика принятия решений
fallback_ratio
0.9
Если не удалось точно распознать цену — ставит
90% от средней цены
. Меньше = дешевле, но быстрее продать.
max_difference_percent
30
Если цена и средняя цена отличаются больше чем на X% — запускается
повторное точное распознавание
.
robust_attempts
12
Сколько раз перечитывать число, если сомневается (при большом отличии). Больше = надёжнее, но дольше.
min_majority_count
4
Сколько раз одно и то же число должно появиться при перечтении, чтобы программа ему поверила.

⏱ sleep — Задержки (в секундах)
between_clicks
0.04 – 0.06
Между кликами по кнопкам
after_recognition
0.04 – 0.06
После распознавания числа
before_input
0.04 – 0.06
Перед вводом цены в поле
after_input
0.14 – 0.16
После ввода цены, перед кликом на "Сделать заказ"
between_cycles
0.4 – 0.6
Пауза между полными циклами (одна продажа → следующая)
robust_recognition
0.02 – 0.03
Между попытками перечтения числа (если сомневается)

Все задержки — случайные в указанном диапазоне, чтобы выглядело как человек. 
